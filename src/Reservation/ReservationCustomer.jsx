...
{['pending', 'rescheduled', 'approved', 'ongoing', 'completed'].map(tab => (            <button
              key={tab}
...
{/* Warning Message for Rescheduled - ADD THIS */}
{activeTab === 'rescheduled' && reservations.length > 0 && (
  <div style={{
    ...
    <p style={{
      ...
      These reservations have been rescheduled and are awaiting approval. You cannot reschedule them again.
    </p>
  </div>
)}
...
{reservation.status === 'approved' ? 'APPROVED' : 
 ...
 reservation.status === 'ongoing' ? 'ONGOING' :
 reservation.status === 'rescheduled' ? 'RESCHEDULED' :  // ✅ ADD THIS
 reservation.status.toUpperCase()}               
                 ...
                 {/* Duration, Amount, Payment, QR Code */}
                  ...
                  {/* Action Buttons */}
                {(reservation.status === 'pending' || reservation.status === 'rescheduled') && (
  <div style={{
    display: 'flex',
    gap: '10px',
    marginTop: '20px'
    ...
    <button
      ...
      disabled={reservation.status === 'rescheduled'}  // ✅ Disable if already rescheduled
      style={{
        padding: '10px 20px',
        backgroundColor: reservation.status === 'rescheduled' ? '#6c757d' : '#17a2b8',  // ✅ Gray if disabled
        color: 'white',
        border: 'none',
        borderRadius: '6px',
        fontSize: '14px',
        fontWeight: '600',
        cursor: reservation.status === 'rescheduled' ? 'not-allowed' : 'pointer',  // ✅ Not-allowed cursor
        opacity: reservation.status === 'rescheduled' ? 0.6 : 1,  // ✅ Lower opacity if disabled
        transition: 'all 0.2s'
      }}
      onMouseEnter={e => {
        if (reservation.status !== 'rescheduled') {
    ...
    >
      {reservation.status === 'rescheduled' ? 'Already Rescheduled' : 'Reschedule'}
    </button>
  </div>
)}
                </div>
